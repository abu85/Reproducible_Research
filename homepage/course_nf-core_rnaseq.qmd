...
Add the `save_reference` line as recommended by nf-core (because we are using their downloaded human genome). You can add it to the json file with `"save_reference": true`. Make sure that all lines but the last one have a comma at the end. 
...
```{.bash}
nextflow run nf-core/rnaseq -r 3.26.0 -resume -params-file nf-params.json
```
...
- we want to `run` the `nf-core/rnaseq` pipeline, version `3.26.0`
...
/your_home_directory/.pixi/bin/pixi run nextflow run nf-core/rnaseq -r 3.26.0 -params-file /your_path/nf-params.json -c server.config
```
