...
```{.bash}
repbio26/common_data/nf-training
```
...
Another important concept in Nextflow. This is the way you ran fastQC on your samples yesterday:

```{.bash}
srun -A project_ID -t 15:00 -n 1 pixi run fastqc --noextract -o fastqc_out data data/sample_1.fastq.gz data/sample_2.fastq.gz 
```
...
First, we initialize a `channel` labeled "greeting_ch". It contains the value from params.greeting. This channel is the input for the processes in the workflow.

Then: execute the process SPLITLETTERS on the greeting_ch (aka greeting channel), and store the output in the channel letters_ch.
