...
<br>
Here we use containers directly for bioinformatics tools with Apptainer, so software runs the same way everywhere. (Module 1 introduced containers as a fix for R dependency problems.)
...
Both let you use a container image, but:

- **`apptainer run`** executes the container's `%runscript` — the default action the image was built to do.
- **`apptainer exec`** ignores the `%runscript` and runs the command you give it.

In this course you will mostly use `exec`, because you usually want to call one specific tool, for example:
